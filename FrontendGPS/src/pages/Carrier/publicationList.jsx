...
function PublicationList() {
    ...
    useEffect(() => {
        const fetchPublications = async () => {
                const url = `http://localhost:3000/publication/filtrar/${encodeURIComponent(filterCity)}`;
    ...
    useEffect(() => {
        const fetchCities = async () => {
            const response = await fetch('http://localhost:3000/publication/ciudadesinicio'); // Paso 3
            const data = await response.json();
            setCities(data); 
        };
    ...
    return (
        <div className="p-4 ml-64">
            ...
            {publications.map((publication) => (
                <a key={publication._id} className="mt-20 flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 mb-2">
